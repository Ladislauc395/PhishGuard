...
class _Settings:
    ...
    # CORRECÇÃO: URLSCAN_API_KEY usa URLSCAN_IO do .env (que tem a chave correta)
    URLSCAN_API_KEY: str = field(
        default_factory=lambda: os.getenv("URLSCAN_IO", "") or os.getenv("URLSCAN_API", "")
    )
    URLSCAN_API: str = field(
        default_factory=lambda: os.getenv("URLSCAN_API", "")
    )
    URLSCAN_IO: str = field(
        default_factory=lambda: os.getenv("URLSCAN_IO", "")
    )
    ...
    DATABASE_URL: str = field(
        default_factory=lambda: os.getenv("DATABASE_URL", "postgresql+psycopg2://ladislau:1234@localhost:5432/phishguard")
    )
    SECRET_KEY: str = field(
        default_factory=lambda: os.getenv("SECRET_KEY", "trocar-em-producao")
    )
    DEBUG: bool = field(
